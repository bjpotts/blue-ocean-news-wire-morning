-- Compare a SHA-256 digest instead of bcrypt so the raw secret is hashed on the
-- client and never written into SQL, a migration file or the database.
-- The secret is 48 characters of URL-safe entropy, so a plain digest is
-- sufficient here; there is no low-entropy guessing surface to slow down.
...
insert into public.ingest_credentials (id, secret_hash)
values (1, 'e39eec8a7ef9173c6e1e22e85c7dd32077de3f79d0cdfb48e072060f9ffe9385')
on conflict (id) do update set secret_hash = excluded.secret_hash, created_at = now();